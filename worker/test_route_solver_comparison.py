...
class RouteSolverComparisonTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self._previous_ortools_limit = os.environ.get("ROUTE_SOLVER_ORTOOLS_TIME_LIMIT_SECONDS")
        os.environ["ROUTE_SOLVER_ORTOOLS_TIME_LIMIT_SECONDS"] = "2"
    ...
    def tearDown(self) -> None:
        ...
        if self._previous_ortools_limit is None:
            os.environ.pop("ROUTE_SOLVER_ORTOOLS_TIME_LIMIT_SECONDS", None)
        else:
            os.environ["ROUTE_SOLVER_ORTOOLS_TIME_LIMIT_SECONDS"] = self._previous_ortools_limit

    def test_auto_compares_legacy_and_ortools_for_less_than_20_nodes(self) -> None:
        rng = random.Random(20260209)
        os.environ["ROUTE_SOLVER_ENGINE"] = "auto"
        has_ortools = _ortools_available()

        for _ in range(20):
            node_count = rng.randint(3, 19)
            ...
            ortools_eval = _evaluate_route(dist, stop_windows, ortools.order) if ortools else None
            ...
            if has_ortools and ortools is not None and ortools.order:
            ...
            if legacy_eval is None and ortools_eval is None:
                self.assertIsNone(auto_eval)
                self.assertIsNone(auto.total_time)
                continue

            self.assertIsNotNone(auto_eval)
            assert auto_eval is not None
            self.assertGreaterEqual(auto.total_time, auto_eval)

            expected_best = min(
                candidate
                for candidate in [legacy_eval, ortools_eval]
                if candidate is not None
            )
            self.assertEqual(auto_eval, expected_best)
